...
public class FeedCacheInvalidationListener {
    ...
    private final Cache<String, FeedPageResponse> feedPublicCache;   // 本地缓存
    ...
    public void onCounterChanged(CounterEvent event) {
        ...
        if ("like".equals(metric) || "fav".equals(metric)) {
            ...
            try {
                ...
                if (post != null && post.getCreatorId() != null) {
                    ...
                    if ("like".equals(metric)) {
                        userCounterService.incrementLikesReceived(owner, delta);  // Lua更新redis
                    }
            ...
            if (cur != null) {
                keys.addAll(cur);  // 获取当前更新的knowpost所出现在的首页的key
            }
    ...
    private void writePageJsonKeepingTtl(String key, FeedPageResponse page) {
        try {
            ...
            if (ttl > 0) {
                redis.opsForValue().set(key, json, Duration.ofSeconds(ttl));  // 保留页面的ttl, 不让redis自动更新ttl
            } else {
