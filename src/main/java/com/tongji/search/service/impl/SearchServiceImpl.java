...
public class SearchServiceImpl implements SearchService {
    ...
    public SearchResponse search(String q, int size, String tagsCsv, String after, Long currentUserIdNullable) {
        ...
        try {
            resp = es.search(s -> {
                var b = s.index(INDEX)
                        ...
                        .query(qb -> qb.functionScore(fs -> fs  // function_score 查询
                                .query(qb2 -> qb2.bool(bq -> {  // bool查询
                                    bq.must(m -> m.multiMatch(mm -> mm.query(q)  // MUST: 必须满足 - 全文搜索
                                            .fields("title^3", "body")));     // 标题权重3倍
                                    bq.filter(f -> f.term(t -> t.field("status") // FILTER: 必须满足 - 状态过滤
                                            .value(v -> v.stringValue("published"))));  // 只查已发布内容

                                    if (tags != null && !tags.isEmpty()) {  // FILTER: 可选 - 标签过滤
                                ...
                                .functions(fn -> fn.fieldValueFactor(fvf -> fvf.field("like_count")
                                        .modifier(FieldValueFactorModifier.Log1p))  // log(1 + like_count)
                                        .weight(2.0))
                                .functions(fn -> fn.fieldValueFactor(fvf -> fvf.field("view_count")
                                        .modifier(FieldValueFactorModifier.Log1p))
                                        .weight(1.0))
                                .boostMode(FunctionBoostMode.Sum)  // 加权方式：相加
                        ))
                        // 返回 title/body 高亮片段，后续合并为 snippet， 用于前端显示
                        .highlight(h -> h
                ...
                if (afterValues != null && !afterValues.isEmpty()) {
                    b = b.searchAfter(afterValues);  // 深度分页
                }
        ...
        }  // 最终得分 = 文本相关性得分 + 2 * log(1 + 点赞数) + 1 * log(1 + 浏览数)
    ...
    /**
     * 联想建议：Completion Suggester，取 title_suggest 的候选文本。 实现用户搜索的自动补全(基于已索引的标题), 就是百度那种补全列表
     */
    @SuppressWarnings("unchecked")
    public SuggestResponse suggest(String prefix, int size) {
        ...
        try {
            resp = es.search(s -> s.index(INDEX)
                    .suggest(sug -> sug.suggesters("title_suggest",  // 建议器名称，搜索字段是 title_suggest, 返回size数量
                ...
                for (var s : entry) {
                    ...
                    if (comp != null && comp.options() != null) {
                        for (var opt : comp.options()) {
                            String text = opt.text();  // 获取建议文本
                            if (text != null && !text.isBlank()) {
                                items.add(text);       // 添加到结果列表
                            }
                        }
                    }
                }
