...
public class RagIndexService {
    ...
    // 向量库封装（Elasticsearch VectorStore），负责写入/检索向量, embedding model用的text-embedding-v4
    ...
    private boolean isUpToDate(long postId, String currentSha, String currentEtag) {
        try {
            ...
            SearchResponse<Map> resp = es.search(s -> s
                            .index(esProps.getIndex())   // 指定索引
                            .size(1)  // 只查第一条
                            .query(q -> q.term(t -> t  // 精确匹配查询
                                    .field("metadata.postId")  // 按元数据的postId查询
                                    .value(v -> v.stringValue(String.valueOf(postId))))),
                    Map.class);
            List<Hit<Map>> hits = resp.hits().hits();
            if (hits == null || hits.isEmpty()) return false;  // 没有找到记录 → 需要重建
            Map source = hits.getFirst().source();             // 获取文档源数据
            if (source == null) return false;
            Object metaObj = source.get("metadata");
            if (!(metaObj instanceof Map<?, ?> meta)) return false;  // metadata 格式检查
            String indexedSha = asString(meta.get("contentSha256")); // 从元数据中提取指纹
            String indexedEtag = asString(meta.get("contentEtag"));  // 从元数据中提取指纹
            if (StringUtils.hasText(currentSha) && StringUtils.hasText(indexedSha)) {
                return Objects.equals(currentSha, indexedSha);       // SHA256一致 → 无需重建
            }
            if (StringUtils.hasText(currentEtag) && StringUtils.hasText(indexedEtag)) {
                return Objects.equals(currentEtag, indexedEtag);     // ETag一致 → 无需重建
            }
            return false;
        } catch (Exception e) {
    ...
    private List<String> chunkMarkdown(String text) {
        ...
        if (!buf.isEmpty()) paras.add(buf.toString());  // 收束最后一段
