...
public class OssStorageService {
    ...
    public String uploadAvatar(long userId, MultipartFile file) {
        ...
        String objectKey = props.getFolder() + "/" + userId + "-" + Instant.now().getEpochSecond() + ext;

        OSS client = new OSSClientBuilder().build(props.getEndpoint(), props.getAccessKeyId(),
                props.getAccessKeySecret());

        try{
            PutObjectRequest putObjectRequest = new PutObjectRequest(props.getBucket(), objectKey, file.getInputStream());
    ...
    private void ensureConfigured() {
        if (props.getEndpoint() == null || props.getAccessKeyId() == null || props.getAccessKeySecret() == null
            ...
            throw new BusinessException(ErrorCode.BAD_REQUEST, "对象存储未设置");
        }
    }
