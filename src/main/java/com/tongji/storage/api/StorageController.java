...
public class StorageController {
    ...
    private final KnowPostMapper knowPostMapper;  // 帖子SQL操作对象
    ...
    public StoragePresignResponse presign(@Valid @RequestBody StoragePresignRequest request,
        ...
        if("knowpost_content".equals(scene)){
            objectKey = "post/" + postId + "/content" + ext;
        } else if ("knowpost_image".equals(scene)){
            String date = DateTimeFormatter.ofPattern("yyyyMMddHHmmss").withZone(ZoneId.of("UTC")).format(Instant.now());
            String rand = UUID.randomUUID().toString().replace("-", "").substring(0, 8);
            objectKey = "post/" + postId + "/images/" + date + "/" + rand + ext;
        }else {
        ...
        Map<String, String> headers = Map.of("Content-Type", request.contentType());  // 以JSON格式存储
